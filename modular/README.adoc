...
There are a few problems with this. One is obvious: all the JavaScript is in a single file (`real-estate-app.js`). Another is more subtle: we have HTML "partials" for views inside our application ("login.html" and "home.html") but they are all in a flat structure and not associated with the controller code that uses them.
...
[source,javascript]
----
angular.module('real-estate-app', [ 'ngRoute' ]).config(
...
and here's the new `real-estate-app.js`:

[source,javascript]
----
code,javascript
angular
    .module('real-estate-app', [ 'ngRoute', 'home', 'navigation' ])
    .config(
...
Notice how the "real-estate-app" module _depends on_ the other two by listing them in the initial declaration along with `ngRoute`. To make that work you just need to load the module definitions in the right order in `index.html`:
...
<script src="js/real-estate-app.js" type="text/javascript"></script>
...
----
...
To complete the application with "natural" routes, you need to tell Angular about it. There are two steps. First, in `real-estate-app.js` you add a line to the `config` function setting the "HTML5 mode" in the `$locationProvider`:

[source,javascript]
----
angular.module('realEstateApp', [ 'ngRoute', 'home', 'navigation' ]).config(
...
<body ng-app="real-estate-app" ng-cloak class="ng-cloak">
...
Suppose you want to make the "auth" module re-usable, so you don't want any hard-coded paths in it. That's not a problem, but you will need to initialize or configure the paths in the `real-estate-app.js` module. To do that you can add a `run()` function:

[source,javascript]
----
angular
  .module('realEstateApp', [ 'ngRoute', 'auth', 'home', 'navigation' ])
  .config(
    ...
  }).run(function(auth) {
...
The `run()` function can call into any of the modules that "realEstateApp" depends on, in this case injecting an `auth` service and initializing it with the paths of the home page, login and logout endpoints respectively.
...
To install the listener you can write a small piece of extra code in your `auth.init()` function (since that is already arranged to run when the main "realEstateApp" module loads):
